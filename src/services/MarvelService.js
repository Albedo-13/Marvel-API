...
const useMarvelService = () => {
  ...
  const getComic = async (id) => {
    ...
    return _transformComic(res.data.results[0]); //res.data.results.map(_transformComic)[0];
  }
  ...
  const _transformComic = (comic) => {
    return {
      ...
      description: comic.description ? `${comic.description.slice(0, 175)}...` : 'There is no information about this comic...',
  ...
  const _transformCharacter = (char) => {
    return {
      id: char.id,
      name: char.name,
      description: char.description ? `${char.description.slice(0, 175)}...` : 'There is no information about this hero...',
